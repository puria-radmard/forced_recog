#!/usr/bin/env python3
"""
Create AT vs UT preference comparison scatter plots from analyze_results_2 CSV files.

This script compares Assistant Tags (AT) vs User Tags (UT) preference accuracy rates
across different treatment categories, excluding recognition data. Creates four plots:
...
def create_at_ut_preference_plot(at_df: pd.DataFrame, ut_df: pd.DataFrame, title: str, output_path: str) -> None:
    """
    Create AT vs UT preference comparison scatter plot.
    
    Args:
        at_df: AT preference DataFrame
        ut_df: UT preference DataFrame
        title: Plot title
    ...
    # Collect data for trendline (only preference data)
    pref_ut_data = []
    pref_at_data = []
    ...
    for row_idx in at_df.index:
        for col in at_df.columns:
            # Only process preference columns
            if col.startswith('pref_'):
                ...
                # Plot point as simple dot (red for preference)
    ...
    # Customize plot
    plt.xlabel('User Tags (UT) Preference Accuracy', fontsize=12)
    plt.ylabel('Assistant Tags (AT) Preference Accuracy', fontsize=12)
    ...
    # Create legend for data types (preference data, trendline, and diagonal)
    ...
    print(f"✅ Created AT vs UT preference plot: {output_path}")


def create_at_ut_preference_dual_trendlines_plot(at_caps_typo_df: pd.DataFrame, ut_caps_typo_df: pd.DataFrame, 
                                                 at_model_comps_df: pd.DataFrame, ut_model_comps_df: pd.DataFrame,
                                                 title: str, output_path: str) -> None:
    """
    Create AT vs UT preference comparison scatter plot with two trendlines.
    ...
    print(f"✅ Created AT vs UT dual preference plot: {output_path}")


def process_at_ut_preference(input_dir: str, output_dir: str) -> None:
    """
    Process AT and UT CSV files (both caps/typo and model comparison) and create preference comparison plots.
    ...
    try:
        ...
        # 1. Caps/Typos only
        create_at_ut_preference_plot(at_caps_typo_df, ut_caps_typo_df, 
                                    "AT vs UT Preference Accuracy (Caps & Typos)", 
                                    os.path.join(output_dir, "AT_vs_UT_preference_caps_typo.png"))
        
        # 2. Model comparisons only
        create_at_ut_preference_plot(at_model_comps_df, ut_model_comps_df, 
                                    "AT vs UT Preference Accuracy (Model Comparisons)", 
                                    os.path.join(output_dir, "AT_vs_UT_preference_model_comps.png"))
        ...
        create_at_ut_preference_plot(at_combined_df, ut_combined_df, 
                                    "AT vs UT Preference Accuracy (All Treatments)", 
                                    os.path.join(output_dir, "AT_vs_UT_preference_all_treatments.png"))
        
        # 4. Dual trendlines (caps/typo vs model comparison)
        create_at_ut_preference_dual_trendlines_plot(at_caps_typo_df, ut_caps_typo_df, 
                                                     at_model_comps_df, ut_model_comps_df,
                                                     "Assistant vs User Tags Preference Accuracy", 
                                                     os.path.join(output_dir, "AT_vs_UT_preference_dual_trendlines.png"))
        
        print(f"\n🎉 AT vs UT preference plots saved to: {output_dir}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Create AT vs UT preference comparison scatter plots')
    ...
    parser.add_argument('--output-dir', 
                       default='results_and_data/analysis/at_ut_preference',
                       help='Directory to save preference comparison plots (default: results_and_data/analysis/at_ut_preference)')
    
    args = parser.parse_args()
    
    print("🎯 Creating AT vs UT Preference Comparison Scatter Plots")
    print("=" * 60)
    print(f"Input directory: {args.input_dir}")
    print(f"Output directory: {args.output_dir}")
    print()
    
    # Process AT vs UT preference comparison
    process_at_ut_preference(args.input_dir, args.output_dir)
