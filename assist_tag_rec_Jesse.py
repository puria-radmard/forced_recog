    ...
    for base_model, model_conversations in conversations_by_model.items():
        ...
        for conv in tqdm(limited_conversations, desc=f"Processing {base_model}"):
            ...
            if experiment_type == "assist_tag":
                ...
                full_detection_prompt = user_prompt_template.format(passage=passage).format(response_1=response_1).format(response_2=response_2)
                conversation_full = chat_wrapper.format_chat(
                    system_prompt=system_prompt,
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        config_path = "configs/assist_tag/assist_tag_config.yaml"
