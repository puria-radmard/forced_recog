...
from scripts.pairwise_selection.pairwise_hf import get_choice_tokens, load_completion_status
            ...
            for non_target_key in non_target_keys:
                ...
                try:
                    ...
                    forwards_followup_prompt = chat_wrapper.format_chat(
                        ...
                        in_context_questions=[user_message_forwards],
                        in_context_answers=[forward_target_choice],
                        user_message=DETECTION_FOLLOWUP_QUESTION.format(idx = forward_target_choice),
                        prefiller=""
                    )
                    ...
                    backwards_followup_prompt = chat_wrapper.format_chat(
                        system_prompt=DETECTION_SYSTEM_PROMPT,
                        in_context_questions=[user_message_backwards],
                        in_context_answers=[backward_target_choice],
                        user_message=DETECTION_FOLLOWUP_QUESTION.format(idx = backward_target_choice),
                        prefiller=""
                    )
...
if __name__ == "__main__":
    ...
    if effective_argc not in [3, 4]:
        print("Usage:")
        print("  Base model: python -m scripts.pairwise_selection.pairwise_hf.py /path/to/yaml/args.yaml temp{temp}_style{style} [continue]")
