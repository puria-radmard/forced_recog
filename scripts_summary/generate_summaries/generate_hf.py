...
from prompts.summary import DATASET_SYSTEM_PROMPTS, SUMMARIZE_PROMPT_TEMPLATES, STYLE_PROMPT_ADDENDUM
...
if __name__ == "__main__":
    ...
    if effective_argc not in [2, 4]:
        print("Usage:")
        print("  Base model: python -m scripts_summary.generate_summaries.generate_hf.py /path/to/yaml/args.yaml [continue]")
        print("  With LoRA:  python -m scripts_summary.generate_summaries.generate_hf.py /path/to/yaml/args.yaml <wandb_run_name> <artifact_suffix> [continue]")
        sys.exit(1)
