...
def main(config_path: str, use_lora: bool = False, lora_run_name: str = None, artifact_name: str = None):
    """
    Main function for choice model analysis - can be called from other scripts_summary.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in [2, 4]:
        print("Usage:")
        print("  Base model: python -m scripts_summary.choice_analysis.analyse_choices /path/to/config.yaml")
        print("  With LoRA:  python -m scripts_summary.choice_analysis.analyse_choices /path/to/config.yaml <wandb_run_name> <artifact_name>")
        sys.exit(1)
