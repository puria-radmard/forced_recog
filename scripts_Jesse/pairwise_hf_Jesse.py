    ...
    for temp, num_trial, style in zip(temps, num_trials, styles):
        for trial_idx in range(num_trial):
            try:
                ...
                print(f"Warning: No summaries found for temp={temp}, trial={trial_idx}, style={style}")
                # Create empty DataFrame as fallback
                all_summaries[(temp, trial_idx, style)] = pd.DataFrame(columns=['document_idx', 'summary'])

    # Limit to specified number of documents
    limited_data = split_data.head(max_documents)
    print(f"Processing limited sample: {len(limited_data)} documents")
    
    for idx, row in tqdm(limited_data.iterrows(), total=len(limited_data), desc=f"Eliciting choices for {split_name}"):
        ...
        if len(summaries) == 0:
            print(f"No summaries found for document {document_idx}, creating mock summaries for testing")
            # Create mock summaries for testing
            summaries = {
                (0.0, 0, 'sun'): f"BREAKING: {article[:50]}... Major developments unfolding.",
                (0.0, 0, 'economist'): f"Recent analysis indicates significant developments regarding {article[:30]}... with broader implications for stakeholders.",
                (0.0, 0, 'natural'): f"So here's what's happening: {article[:40]}... Pretty wild stuff!"
            }
        ...
        # Limit to specified number of pairs per document
        if len(expected_pairs) > max_pairs:
            expected_pairs = set(list(expected_pairs)[:max_pairs])
            print(f"Limited to {max_pairs} pairs for document {document_idx} (from {len(all_keys)} summaries)")
...
if __name__ == "__main__":
    ...
    if effective_argc not in [2, 3, 4, 5]:
        print("Usage:")
        print("  Base model: python pairwise_hf_Jesse.py <config_path> [max_documents] [max_pairs] [continue]")
        print("  With LoRA:  python pairwise_hf_Jesse.py <config_path> <wandb_run_name> <artifact_name> [max_documents] [max_pairs] [continue]")
        print("Examples:")
        print("  python pairwise_hf_Jesse.py configs/test_config.yaml 2 1")
        print("  python pairwise_hf_Jesse.py configs/test_config.yaml 5 2 continue")
        sys.exit(1)
    ...
    if len(sys.argv) > 2 and sys.argv[2].isdigit():
        ...
        wandb_run_name = None
        artifact_name = None
        max_documents = int(sys.argv[2]) if len(sys.argv) > 2 else 2
        max_pairs = int(sys.argv[3]) if len(sys.argv) > 3 and sys.argv[3].isdigit() else 1
    else:
        # LoRA model
        use_lora = True
        wandb_run_name = sys.argv[2] if len(sys.argv) > 2 else None
        artifact_name = sys.argv[3] if len(sys.argv) > 3 else None
        max_documents = int(sys.argv[4]) if len(sys.argv) > 4 and sys.argv[4].isdigit() else 2
        max_pairs = int(sys.argv[5]) if len(sys.argv) > 5 and sys.argv[5].isdigit() else 1
