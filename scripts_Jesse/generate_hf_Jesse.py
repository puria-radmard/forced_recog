...
def generate_summaries_for_split(
    ...
    for idx, row in tqdm(split_data.iterrows(), total=len(split_data), desc=f"Processing {split_name}"):
        ...
        # Debug: Print the first few characters of the prompt
        if idx < 2:  # Only for first 2 documents
            print(f"Debug - Document {idx}:")
        ...
        for temp, num_trial, style in zip(temps, num_trials, styles):
            ...
            for trial_idx in range(num_trial):
                ...
                extra_chat_with_tags = chat_wrapper.format_chat(user_message = extra_chat, prefiller="").removeprefix(chat_wrapper.format_chat(user_message="-"))
                ...
                # Debug: Print generation result for first few documents
                if idx < 2:  # Only for first 2 documents
                    print(f"Debug - Generated text: '{generation_result['generated_texts'][0]}'")
                    print(f"Debug - Processed summary: '{summary}'")
